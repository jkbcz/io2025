...
\begin{figure}[h]
    \centering
    % TODO: Tutaj wstawisz nazwę swojego pliku z diagramem, np. diagram_uc.png
    % \includegraphics[width=0.9\textwidth]{diagram_uc.png}
    \caption{Diagram przypadków użycia systemu informacji pasażerskiej.}
    \label{fig:use_case}
\end{figure}
...
\begin{figure}[h]
    \centering
    % TODO: Wgraj plik o nazwie diagram4.png 
    % \includegraphics[width=0.9\textwidth]{diagram4.png}
...
\begin{itemize}
    \item \textbf{Linia:} Reprezentuje konkretny numer linii (np. "Tramwaj 8"). Przechowuje informacje o trasie i typie pojazdu.
    \item \textbf{Przystanek:} Punkt na mapie, do którego przypisane są odjazdy. Zawiera nazwę, lokalizację GPS oraz listę linii, które go obsługują.
    \item \textbf{Kurs (Trip):} Konkretna realizacja przejazdu danej linii o określonej godzinie. To do kursu przypisane są dane o opóźnieniach.
    \item \textbf{Pojazd:} Fizyczna jednostka (autobus/tramwaj) przypisana do kursu, przesyłająca swoje współrzędne geograficzne.
    \item \textbf{Zdarzenie drogowe:} Informacja o utrudnieniach (korki, awarie), która wpływa na estymację czasu dojazdu.
\end{itemize}
...
\begin{itemize}
    ...
    \item Każdy \textbf{Kurs} obsługuje wiele \textbf{Przystanków} w określonej kolejności (harmonogram).
    \item \textbf{Pojazd} jest przypisany do dokładnie jednego \textbf{Kursu} w danym momencie.
    \item \textbf{Zdarzenie drogowe} może wpływać na wiele \textbf{Kursów} przechodzących przez dany obszar.
\end{itemize}
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|p{10cm}|}
        \hline
        \textbf{Nazwa przypadku} & Dodanie komunikatu o utrudnieniach drogowych                                            \\ \hline
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|p{10cm}|}
        \hline
        \textbf{Nazwa przypadku} & Przeglądanie statycznego rozkładu linii \\ \hline
        \textbf{Aktor główny}    & Pasażer                                 \\ \hline
        \textbf{Przebieg główny} &
        ...
        4. System generuje tabelaryczny rozkład jazdy (godziny i minuty) na dany dzień tygodnia. \newline
        5. System zaznacza na rozkładzie kursy niskopodłogowe.             \\ \hline
    \end{tabular}
\end{table}
...
\subsection{Scenariusz 7: Obsługa kursu wypadającego z rozkładu}

\begin{table}[h]
    \centering
    \begin{tabular}{|l|p{10cm}|}
        \hline
        \textbf{Nazwa przypadku} & Automatyczne wycofanie kursu z tablicy Live     \\ \hline
        \textbf{Aktor główny}    & System (Proces automatyczny)                    \\ \hline
        \textbf{Opis}            &
        1. System wykrywa brak sygnału GPS z pojazdu przez czas dłuższy niż 5 minut lub otrzymuje sygnał o awarii z komputera pokładowego. \newline
        2. System oznacza kurs jako „Niedostępny/Awaria”. \newline
        3. Algorytm ETA usuwa ten kurs z list odjazdów na wszystkich przystankach na trasie. \newline
        4. System wysyła powiadomienie do dyspozytora w celu weryfikacji sytuacji. \\ \hline
    \end{tabular}
\end{table}
...
\begin{figure}[h]
    \centering
    % TODO: Wgraj plik o nazwie dfd_level0.png
    % \includegraphics[width=0.7\textwidth]{dfd_level0.png}
...
\subsection{Dekompozycja procesów (Poziom 1)}
Na tym poziomie system został rozbity na główne procesy funkcjonalne.

\begin{figure}[h]
    \centering
    % TODO: Wgraj plik o nazwie dfd_level1.png 
...
\begin{enumerate}
    \item \textbf{P1. Zarządzanie bazą rozkładów:} Przechowywanie statycznych godzin odjazdów.
    \item \textbf{P2. Moduł śledzenia GPS:} Odbiór i normalizacja współrzędnych z pojazdów.
    \item \textbf{P3. Algorytm Estymacji (ETA):} Proces łączący dane statyczne, pozycję GPS oraz dane Traffic w celu obliczenia faktycznego opóźnienia.
...
\begin{itemize}
    \item \textbf{D1 Baza Rozkładów:} Dane statyczne o liniach i przystankach.
    \item \textbf{D2 Status Pojazdów:} Dynamiczne dane o aktualnej pozycji i prędkości.
    \item \textbf{D3 Rejestr Komunikatów:} Baza aktywnych utrudnień i ogłoszeń.
\end{itemize}
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|l|p{7cm}|}
        ...
        Dane Traffic             & Traffic API $\rightarrow$ P3 & ID odcinka drogi, współczynnik zakorkowania (0-1), średnia prędkość.       \\ \hline
        Wynik ETA                & P3 $\rightarrow$ P4          & ID kursu, ID przystanku, przewidywana minuta przyjazdu, status opóźnienia. \\ \hline
        Zapytanie pasażera       & Pasażer $\rightarrow$ P4     & ID przystanku lub numer linii, koordynaty użytkownika.                     \\ \hline
...
\begin{itemize}
    \item \textbf{P3.1 (Oblicz odległość):} Na podstawie GPS pojazdu i współrzędnych przystanku wylicza dystans pozostały do przejechania.
    \item \textbf{P3.2 (Koryguj o ruch):} Mnoży bazowy czas przejazdu przez współczynnik opóźnienia pobrany z modułu Traffic.
    \item \textbf{P4.1 (Sortuj odjazdy):} Układa kursy chronologicznie według wyliczonego czasu przyjazdu.
\end{itemize}
